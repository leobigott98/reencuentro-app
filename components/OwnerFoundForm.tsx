...
export function OwnerFoundForm({ publicCode, token }: { publicCode: string; token: string }) {
  ...
  return (
    <form action={action} className="mt-5 space-y-3 rounded-3xl bg-amber-50 p-4 ring-1 ring-amber-200">
      ...
      <p className="text-sm text-amber-900">Puedes solicitar marcar este caso como encontrado. Para evitar falsos positivos, quedará como “localización en verificación” hasta que un moderador lo confirme.</p>
      ...
      <SubmitButton>Solicitar marcar como encontrado/a</SubmitButton>
    </form>
  );
}
