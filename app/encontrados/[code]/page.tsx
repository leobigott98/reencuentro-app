...
function isDeceasedStatus(status: string) {
...
export default async function FoundRecordPage({
  ...
  if (record.status === "minor_temporary_care" || record.status === "minor_unaccompanied") {
    redirect(`/menores/${record.public_code}`);
  }
  const isDeceased = isDeceasedStatus(record.status);
  ...
  if (isDeceased) {
    return (
      <main className="mx-auto max-w-5xl px-4 py-6">
        <Link href="/buscar" className="text-sm font-bold text-cerca-700">
          ← Volver a búsqueda
        </Link>
        <div className="mt-4 rounded-3xl bg-amber-50 p-4 text-sm text-amber-950 ring-1 ring-amber-200">
          <p className="font-black">Ficha de alto resguardo</p>
          <p className="mt-1">
            Este registro corresponde a una persona fallecida o fallecida por identificar. Por dignidad y privacidad no se muestran fotos, nombre, documento completo ni detalles gráficos.
          </p>
        </div>
        <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_380px]">
          <section className="rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:p-6">
            <p className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-sm font-black text-slate-800">
              {statusLabels[record.status] || record.status}
            </p>
            <h1 className="mt-3 text-3xl font-black sm:text-5xl">Persona fallecida por identificar</h1>
            <p className="mt-2 text-sm text-slate-600">Código público: {record.public_code}</p>
            <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
              ...
              <div className="rounded-2xl bg-slate-50 p-3">
                ...
                <dd>{record.apparent_gender ? genderLabels[record.apparent_gender] || record.apparent_gender : "No indicado"}</dd>
              </div>
              <div className="rounded-2xl bg-slate-50 p-3">
                <dt className="font-bold">Zona general de recuperación</dt>
                <dd>{field(record.found_location)}</dd>
              </div>
              <div className="rounded-2xl bg-slate-50 p-3">
                <dt className="font-bold">Institución o resguardo</dt>
                <dd>{field(record.current_location)}</dd>
              </div>
              <div className="rounded-2xl bg-slate-50 p-3 sm:col-span-2">
                <dt className="font-bold">Fecha registrada</dt>
                <dd>{formatDate(record.found_at || record.created_at)}<span className="mt-1 block text-xs text-slate-500">{relativeTime(record.found_at || record.created_at)}</span></dd>
              </div>
            </dl>
            <p className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
              Si crees reconocer a esta persona, solicita una revisión. El equipo validará la información de forma privada antes de contactar a familiares o comunidades.
            </p>
          </section>
          <aside>
            <FoundRecordActions
              foundRecordId={record.id}
              identityTitle="Solicitar revisión / creo reconocer a esta persona"
              identityDescription="Comparte datos verificables. No subas ni solicites imágenes gráficas por este formulario."
            />
            <div className="mt-4">
              <ReportProblemForm subjectType="found_record" subjectId={record.id} />
            </div>
          </aside>
        </div>
      </main>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_380px]">
        <section className="rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:p-6">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Fuente: {record.source_name || "No indicada"} · {record.created_by_name ? "voluntario registrado" : "fuente comunitaria"}
          </p>
