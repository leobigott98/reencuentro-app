    ...
    <main className="mx-auto max-w-md px-4 py-10">
      <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <Link
            href="/entrar"
            className="rounded-2xl bg-cerca-600 px-5 py-3 text-center font-black text-white"
          >
            Entrar con código
          </Link>
          <Link
            href="/voluntarios/registro"
            className="rounded-2xl bg-slate-100 px-5 py-3 text-center font-black text-slate-800"
          >
            Registrarme
          </Link>
        </div>
...
export default async function VolunteerDashboardPage() {
  ...
  const [{ data: profile }, { data: foundRecords }, { data: uploadBatches }] =
    await Promise.all([
      db
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl font-black">
            {volunteerProfile?.full_name || "Acceso administrador"}
          </h1>
          <p className="text-slate-600">{session.email}</p>
        </div>
        <form action={logout}>
          <button className="rounded-2xl bg-slate-200 px-4 py-2 font-bold">
            Salir
          </button>
        </form>
      </div>
      ...
      <section className="mb-6 grid gap-3 sm:grid-cols-3">
        ...
        >
          ...
          <p className="mt-1 text-sm text-slate-600">
            Reporta una persona encontrada o carga un listado.
          </p>
        </Link>
        <Link
          href="/encontrados/listas"
          className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200"
        >
          <p className="font-black">Listas cargadas</p>
          <p className="mt-1 text-sm text-slate-600">
            Revisa listados y registros asociados a cargas.
          </p>
        </Link>
        <Link
          href="/menores/cuidado-temporal"
          className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200"
        >
          <p className="font-black">Menores en cuidado temporal</p>
          <p className="mt-1 text-sm text-slate-600">
            Accede al flujo para cuidado temporal de menores.
          </p>
        </Link>
      </section>
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <div>
              ...
              <dd className="font-black text-slate-900">
                {volunteerProfile?.role || session.role}
              </dd>
            </div>
            <div>
              <dt className="font-bold text-slate-500">Estado</dt>
              <dd>{volunteerProfile?.verification_status || "admin"}</dd>
            </div>
            <div>
              <dt className="font-bold text-slate-500">Zona</dt>
              <dd>{volunteerProfile?.zone || "No indicada"}</dd>
            </div>
            <div>
              <dt className="font-bold text-slate-500">Teléfono</dt>
              <dd>{volunteerProfile?.phone || "No indicado"}</dd>
            </div>
            <div>
              <dt className="font-bold text-slate-500">Organización</dt>
              <dd>{volunteerProfile?.organization_name || "Independiente"}</dd>
            </div>
            <div>
              <dt className="font-bold text-slate-500">Centro</dt>
              <dd>{volunteerProfile?.center_name || "No indicado"}</dd>
            </div>
            <div>
              <dt className="font-bold text-slate-500">Puntaje de confianza</dt>
              <dd>{volunteerProfile?.trust_score ?? 0}</dd>
            </div>
          </dl>
        </aside>
            ...
            <div className="mb-3 flex items-end justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-600">
                  Personas encontradas asociadas a tu correo.
                </p>
              </div>
            ...
            <div className="grid gap-3">
              {!records.length ? (
                <p className="rounded-3xl bg-white p-4 text-sm text-slate-600 ring-1 ring-slate-200">
                  Todavía no hay registros creados por este correo.
                </p>
              ) : null}
              {records.map((record) => (
                <article
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <h3 className="text-lg font-black">
                        {record.full_name || "Sin identificar"}
                      </h3>
                    </div>
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    {record.current_location || record.found_location || "Sin ubicación"}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    Fuente: {record.source_name || "No indicada"} · Código: {record.public_code}
                  </p>
                </article>
              ))}
            </div>
          ...
          <section>
            <div className="mb-3 flex items-end justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-600">
                  Cargas masivas asociadas a tu correo.
                </p>
              </div>
              <span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-black text-slate-700">
                {batches.length}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {!batches.length ? (
                <p className="rounded-3xl bg-white p-4 text-sm text-slate-600 ring-1 ring-slate-200 sm:col-span-2">
                  Todavía no hay lotes cargados por este correo.
                </p>
              ) : null}
              {batches.map((batch) => (
                <Link
                ...
                >
                  <p className="text-xs font-black uppercase text-cerca-700">
                    {shortDate(batch.created_at)}
                  </p>
                  <h3 className="font-black">
                    {batch.source_name || "Fuente sin nombre"}
                  </h3>
                  <p className="text-sm text-slate-600">
                    {batch.source_location || "Sin ubicación"}
                  </p>
